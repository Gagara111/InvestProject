...
var connectionStringBuilder = new SqliteConnectionStringBuilder(); // создаем экземпляр ConnectionStringBuilder
connectionStringBuilder.DataSource = "MainDate.db"; // задаем имя источника данных

var connection = new SqliteConnection(connectionStringBuilder.ConnectionString);// вводим имя переменной
connection.Open();//открываем соединение

//подготовим команду которую отправляем в б/д
var comand = new SqliteCommand(); // Создаем переменную для команды 
//устанавливаем связь б/д по данному соединению  для команды
comand.Connection = connection;
comand.CommandType = CommandType.Text;
comand.CommandText = @"CREATE TABLE [MainTable] (
